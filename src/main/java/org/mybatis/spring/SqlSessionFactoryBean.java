 ...
 * 创建MyBatis {@code SqlSessionFactory}。这是设置共享的常用方法
 * 在Spring应用程序上下文中使用MyBatis {@code SqlSessionFactory};然后，可以通过依赖项注入将SqlSessionFactory传递给基于mybatison的dao。
 * <p>
 ...
 * {@code DataSourceTransactionManager}或{@code JtaTransactionManager}都可以与{@code SqlSessionFactory}组合用于事务界定。JTA应该用于跨多个数据库的事务，或者当使用容器管理事务(container managed transactions, CMT)时。
 * <p>
 * SqlSessionFactoryBean:实际上是一个右spring处理后的SqlSessionFactory
 *
